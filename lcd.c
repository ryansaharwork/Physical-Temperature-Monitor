...
void lcd_init(void)
{
    ...
    DDRB |= ((1 << PB0) | (1 << PB1)); // Take care not to affect any unnecessary bits
...
void lcd_writenibble(unsigned char lcdbits)
{
    ...
    /* This isn't working as intended for some reason... (this was shown in slides)
    ...
    PORTD |= ((lcdbits & 0x0F) << 4);

    /* Make E signal (PB1) go to 1 and back to 0 */
    PORTB |= (1 << PB1);  // Set e to 1
    PORTB |= (1 << PB1);  // THIS FIXED QUICK BUTTON DELAY ISSUE!!!        
